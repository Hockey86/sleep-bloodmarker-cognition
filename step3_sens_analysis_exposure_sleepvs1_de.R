...
med.res.path <- '/data/haoqisun/inflammation-sleep-dementia/mediation-analysis/mediation_results_exposure_sleepvs1_mediator_biomarkervs1_outcomev2_refined.xlsx'
...
for (i in 1:nrow(df.med)) {
  ...
  sens.res <- medsens(med.res, rho.by=0.01, effect.type='direct', sims=1000)
  ...
  sink(sprintf("/data/haoqisun/inflammation-sleep-dementia/mediation-analysis/sens-de-age-%s-%s.txt", exposure, outcome))
  print(med.mod.age)
  sink()
  
  med.mod.bmi <- test.modmed(med.res,covariates.1=list(BMI_VS1=25),covariates.2=list(BMI_VS1=30),sims=sims)
  sink(sprintf("/data/haoqisun/inflammation-sleep-dementia/mediation-analysis/sens-de-BMI-%s-%s.txt", exposure, outcome))
  print(med.mod.bmi)
  sink()
}
saveRDS(sens.res.all, "/data/haoqisun/inflammation-sleep-dementia/mediation-analysis/sens.res.all_exposure_sleepvs1_mediator_biomarkervs1_outcomev2.rds")
